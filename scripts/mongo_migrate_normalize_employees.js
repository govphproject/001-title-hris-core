...
cursor.forEach((doc) => {
  ...
  if (
    doc.preferredname !== undefined &&
    doc.preferredname !== null &&
    !doc.preferred_name
  ) {
...
const missingCursor = db[collName].find({
  $or: [{ employee_id: { $exists: false } }, { employee_id: null }],
});
let genCount = 0;
missingCursor.forEach((doc) => {
  ...
  if (doc.employeeid) {
    ...
    db[collName].updateOne(
      { _id: doc._id },
      { $set: { employee_id: doc.employeeid }, $unset: { employeeid: "" } }
    );
    genCount += 1;
    return;
  }
...
try {
  db[collName].createIndex(
    ...
    {
      unique: true,
      partialFilterExpression: { employee_id: { $exists: true, $ne: null } },
    }
  );
